...
from pybrops.breed.prot.sel.prob.WeightedGenomicSelectionProblem import WeightedGenomicRealSelectionProblem
...
def prob(
    ...
    yield WeightedGenomicRealSelectionProblem(
        wgebv = gwgebv,
        ndecn = ndecn,
...
def test_RealConventionalSelectionProblem_docstring():
    assert_class_documentation(WeightedGenomicRealSelectionProblem)
...
def test_nlatent_is_concrete():
    assert_property_isconcrete(WeightedGenomicRealSelectionProblem, "nlatent")
...
def test_gwgebv_is_concrete():
    assert_property_isconcrete(WeightedGenomicRealSelectionProblem, "gwgebv")
...
def test_init_is_concrete():
    assert_method_isconcrete(WeightedGenomicRealSelectionProblem, "__init__")

################
### latentfn ###
################
def test_latentfn_is_concrete(prob):
    assert_method_isconcrete(WeightedGenomicRealSelectionProblem, "latentfn")
...
def test_from_numpy(
    ...
    # construct problem
    gebvprob = WeightedGenomicRealSelectionProblem.from_numpy(
        Z_a, u_a, fafreq, 
        ndecn, decn_space, decn_space_lower, decn_space_upper, 
        nobj, obj_wt, obj_trans, obj_trans_kwargs, 
        nineqcv, ineqcv_wt, ineqcv_trans, ineqcv_trans_kwargs, 
...
def test_from_gmat_algpmod(
    ...
    gwgebv = Z_a.dot(u_a * numpy.power(fafreq, -0.5))
    # construct problem
    gebvprob = WeightedGenomicRealSelectionProblem.from_gmat_algpmod(
        gmat, gpmod,
        ndecn, decn_space, decn_space_lower, decn_space_upper, 
        nobj, obj_wt, obj_trans, obj_trans_kwargs, 
        nineqcv, ineqcv_wt, ineqcv_trans, ineqcv_trans_kwargs, 
