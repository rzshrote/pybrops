...
def prob(
    ...
    yield L2NormGenomicIntegerSelectionProblem(
        C = C,
        ndecn = ndecn,
...
def test_latentfn_is_concrete(prob):
    assert_method_isconcrete(L2NormGenomicIntegerSelectionProblem, "latentfn")
...
def test_from_gmat(
    ...
    # construct problem
    l1gsprob = L2NormGenomicIntegerSelectionProblem.from_gmat(
        gmat, cmatfcty, mkrwt, afreq,
        ndecn, decn_space, decn_space_lower, decn_space_upper, 
        nobj, obj_wt, obj_trans, obj_trans_kwargs, 
        nineqcv, ineqcv_wt, ineqcv_trans, ineqcv_trans_kwargs, 
