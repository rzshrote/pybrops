...
from pybrops.breed.prot.sel.prob.L1NormGenomicSelectionProblem import L1NormGenomicRealSelectionProblem
...
def decn_space_lower(ndecn):
    yield numpy.repeat(0.0, ndecn)

@pytest.fixture
def decn_space_upper(ndecn):
    yield numpy.repeat(1.0, ndecn)
...
def prob(
    ...
    yield L1NormGenomicRealSelectionProblem(
        V = V,
        ndecn = ndecn,
...
def test_RealConventionalSelectionProblem_docstring():
    assert_class_documentation(L1NormGenomicRealSelectionProblem)
...
def test_nlatent_is_concrete():
    assert_property_isconcrete(L1NormGenomicRealSelectionProblem, "nlatent")
...
def test_V_is_concrete():
    assert_property_isconcrete(L1NormGenomicRealSelectionProblem, "V")
...
def test_init_is_concrete():
    assert_method_isconcrete(L1NormGenomicRealSelectionProblem, "__init__")

################
### latentfn ###
################
def test_latentfn_is_concrete(prob):
    assert_method_isconcrete(L1NormGenomicRealSelectionProblem, "latentfn")

def test_latentfn(prob, ndecn, V):
...
def test_from_bvmat(
    ...
    # construct problem
    l1gsprob = L1NormGenomicRealSelectionProblem.from_numpy(
        mkrwt, tafreq, tfreq,
        ndecn, decn_space, decn_space_lower, decn_space_upper, 
        nobj, obj_wt, obj_trans, obj_trans_kwargs, 
        nineqcv, ineqcv_wt, ineqcv_trans, ineqcv_trans_kwargs, 
