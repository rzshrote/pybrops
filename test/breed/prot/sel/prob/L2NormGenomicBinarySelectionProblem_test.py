...
def prob(
    ...
    yield L2NormGenomicBinarySelectionProblem(
        C = C,
        ndecn = ndecn,
...
def test_BinaryConventionalSelectionProblem_docstring():
    assert_class_documentation(L2NormGenomicBinarySelectionProblem)
...
def test_nlatent_is_concrete():
    assert_property_isconcrete(L2NormGenomicBinarySelectionProblem, "nlatent")
...
def test_C_is_concrete():
    assert_property_isconcrete(L2NormGenomicBinarySelectionProblem, "C")
...
def test___init___is_concrete():
    assert_method_isconcrete(L2NormGenomicBinarySelectionProblem, "__init__")

################
### latentfn ###
################
def test_latentfn_is_concrete(prob):
    assert_method_isconcrete(L2NormGenomicBinarySelectionProblem, "latentfn")

def test_latentfn(prob, ndecn, C):
    x = numpy.random.binomial(1, 0.5, ndecn)
    y = (1.0 / x.sum()) * x
    a = prob.latentfn(x)
    b = prob.latentfn(2.5*x)
...
def test_from_gmat(
    ...
    # construct problem
    l1gsprob = L2NormGenomicBinarySelectionProblem.from_gmat(
        gmat, cmatfcty, mkrwt, afreq,
        ndecn, decn_space, decn_space_lower, decn_space_upper, 
        nobj, obj_wt, obj_trans, obj_trans_kwargs, 
        nineqcv, ineqcv_wt, ineqcv_trans, ineqcv_trans_kwargs, 
