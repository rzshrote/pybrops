...
def decn_space_upper(ntaxa):
    yield numpy.repeat(ntaxa-1, ntaxa)
...
def prob(
    ...
    yield FamilyEstimatedBreedingValueSubsetSelectionProblem(
        ebv = ebv,
        familyid = familyid,
        ndecn = ndecn,
...
def test_SubsetConventionalSelectionProblem_docstring():
    assert_class_documentation(FamilyEstimatedBreedingValueSubsetSelectionProblem)
...
def test_nlatent_is_concrete():
    assert_property_isconcrete(FamilyEstimatedBreedingValueSubsetSelectionProblem, "nlatent")
...
def test_ebv_is_concrete():
    assert_property_isconcrete(FamilyEstimatedBreedingValueSubsetSelectionProblem, "ebv")
...
def test_init_is_concrete():
    assert_method_isconcrete(FamilyEstimatedBreedingValueSubsetSelectionProblem, "__init__")

################
### latentfn ###
################
def test_latentfn_is_concrete(prob):
    assert_method_isconcrete(FamilyEstimatedBreedingValueSubsetSelectionProblem, "latentfn")

def test_latentfn(prob, ntaxa, ebv, familyid):
    x = numpy.random.choice(ntaxa, ntaxa // 2)
    a = prob.latentfn(x)
    b = -(1.0/len(x)) * ebv[x,:].sum(0)
    familywt = numpy.zeros(len(familyid), dtype = float)
    familywt[x] = (1.0/len(x))
    c = -numpy.bincount(familyid, familywt)
...
def test_from_bvmat(
    ...
    # construct problem
    ebvprob = FamilyEstimatedBreedingValueSubsetSelectionProblem.from_bvmat(
        bvmat, 
        ndecn, decn_space, decn_space_lower, decn_space_upper, 
        nobj, obj_wt, obj_trans, obj_trans_kwargs, 
        nineqcv, ineqcv_wt, ineqcv_trans, ineqcv_trans_kwargs, 
