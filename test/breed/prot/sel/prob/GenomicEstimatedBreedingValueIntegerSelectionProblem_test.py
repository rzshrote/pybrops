...
def decn_space_upper(ntaxa):
    yield numpy.repeat(ntaxa, ntaxa)
...
def prob(
    ...
    yield GenomicEstimatedBreedingValueIntegerSelectionProblem(
        gebv = gebv,
        ndecn = ndecn,
...
def test_IntegerConventionalSelectionProblem_docstring():
    assert_class_documentation(GenomicEstimatedBreedingValueIntegerSelectionProblem)
...
def test_nlatent_is_concrete():
    assert_property_isconcrete(GenomicEstimatedBreedingValueIntegerSelectionProblem, "nlatent")
...
def test_gebv_is_concrete():
    assert_property_isconcrete(GenomicEstimatedBreedingValueIntegerSelectionProblem, "gebv")
...
def test_init_is_concrete():
    assert_method_isconcrete(GenomicEstimatedBreedingValueIntegerSelectionProblem, "__init__")

################
### latentfn ###
################
def test_latentfn_is_concrete(prob):
    assert_method_isconcrete(GenomicEstimatedBreedingValueIntegerSelectionProblem, "latentfn")

def test_latentfn(prob, ntaxa, gebv):
    x = numpy.random.binomial(ntaxa, 0.5, ntaxa)
    x = (1.0 / x.sum()) * x
    a = prob.latentfn(x)
    b = -x.dot(gebv)
...
def test_from_bvmat(
    ...
    # construct problem
    gebvprob = GenomicEstimatedBreedingValueIntegerSelectionProblem.from_bvmat(
        bvmat, unscale,
        ndecn, decn_space, decn_space_lower, decn_space_upper, 
        nobj, obj_wt, obj_trans, obj_trans_kwargs, 
        nineqcv, ineqcv_wt, ineqcv_trans, ineqcv_trans_kwargs, 
...
def test_from_gmat_gpmod(
    ...
    # construct problem
    gebvprob = GenomicEstimatedBreedingValueIntegerSelectionProblem.from_gmat_gpmod(
        gmat, gpmod, unscale,
        ndecn, decn_space, decn_space_lower, decn_space_upper, 
        nobj, obj_wt, obj_trans, obj_trans_kwargs, 
        nineqcv, ineqcv_wt, ineqcv_trans, ineqcv_trans_kwargs, 
