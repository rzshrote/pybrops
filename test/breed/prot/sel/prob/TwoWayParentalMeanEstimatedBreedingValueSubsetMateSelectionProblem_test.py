...
def test___init___is_concrete():
    assert_method_isconcrete(TwoWayParentalMeanEstimatedBreedingValueSubsetMateSelectionProblem, "__init__")

def test___init__(selprob):
    assert isinstance(selprob, TwoWayParentalMeanEstimatedBreedingValueSubsetMateSelectionProblem)
...
def test_latentfn_is_concrete():
    assert_method_isconcrete(TwoWayParentalMeanEstimatedBreedingValueSubsetMateSelectionProblem, "latentfn")

def test_latentfn(selprob,selprob_ndecn,selprob_xmap,ntrait):
    x = numpy.random.choice(len(selprob_xmap), selprob_ndecn)
    out = selprob.latentfn(x)
    assert isinstance(out, numpy.ndarray)
    assert len(out) == ntrait
