...
def prob(
    ...
    yield ExpectedMaximumBreedingValueSubsetSelectionProblem(
        embv = embv,
        ndecn = ndecn,
...
def test_SubsetConventionalSelectionProblem_docstring():
    assert_class_documentation(ExpectedMaximumBreedingValueSubsetSelectionProblem)
...
def test_nlatent_is_concrete():
    assert_property_isconcrete(ExpectedMaximumBreedingValueSubsetSelectionProblem, "nlatent")
...
def test_embv_is_concrete():
    assert_property_isconcrete(ExpectedMaximumBreedingValueSubsetSelectionProblem, "embv")
...
def test_init_is_concrete():
    assert_method_isconcrete(ExpectedMaximumBreedingValueSubsetSelectionProblem, "__init__")

################
### latentfn ###
################
def test_latentfn_is_concrete(prob):
    assert_method_isconcrete(ExpectedMaximumBreedingValueSubsetSelectionProblem, "latentfn")

def test_latentfn(prob, ndecn, embv):
    x = numpy.random.choice(ndecn, ndecn // 2)
    a = prob.latentfn(x)
    b = -(1.0/len(x)) * embv[x,:].sum(0)
...
def test_from_pgmat_gpmod(
    ...
    # construct problem
    gebvprob = ExpectedMaximumBreedingValueSubsetSelectionProblem.from_pgmat_gpmod(
        nparent, ncross, nprogeny, nrep, unique_parents, pgmat, gpmod, mateprot,
        ndecn, decn_space, decn_space_lower, decn_space_upper, 
        nobj, obj_wt, obj_trans, obj_trans_kwargs, 
        nineqcv, ineqcv_wt, ineqcv_trans, ineqcv_trans_kwargs, 
    ...
    b = -(1.0/len(x)) * gebvprob.embv[x,:].sum(0)
    assert numpy.all(numpy.isclose(a,b))
