"""
Module implementing a subset genetic algorithm adapted with specialized operators for large scale optimization.
"""

# all public classes and functions available in this module
__all__ = [
    "LargeScaleSubsetGeneticAlgorithm",
]
...
from pybrops.opt.algo.pymoo_addon_large_scale import OrderedSubsetPoolIterator
from pybrops.opt.algo.pymoo_addon_large_scale import SubsetPoolReducedExchangeMutation
from pybrops.opt.algo.pymoo_addon_large_scale import TiledSubsetPoolIterator
...
class LargeScaleSubsetGeneticAlgorithm(
        SubsetOptimizationAlgorithm,
    ):
    """
    Class implementing a largs scale subset genetic algorithm. 
    The search space is discrete and nominal in nature.
    """
    ...
    def minimize(
        ...
        # construct a random sampling iterator
        it = TiledSubsetPoolIterator(setspace = prob.decn_space)

        # construct sampling operator
        sampling = SubsetPoolRandomSampling(pooliter = it)

        # construct crossover operator
        crossover = ReducedExchangeCrossover()

        # construct mutation operator
        mutation = SubsetPoolReducedExchangeMutation(pooliter = it)
...
class OrderedLargeScaleSubsetGeneticAlgorithm(
        SubsetOptimizationAlgorithm,
    ):
    """
    Class implementing a largs scale subset genetic algorithm. 
    ...
    ############################ Object Properties #############################
    @property
    def orderfn(self) -> Callable:
        """orderfn."""
        return self._orderfn
    @orderfn.setter
    def orderfn(self, value: Callable) -> None:
        """Set orderfn."""
        check_is_Callable(value, "orderfn")
        self._orderfn = value
    ...
    def minimize(
        ...
        # ordere the set space
        orderedsetspace = self.orderfn(prob)

        # construct an ordered sampling iterator
        it = OrderedSubsetPoolIterator(setspace = orderedsetspace)
