...
# calculate results
results, history = gs.pa_sel(
    geno = tarspot_geno,
    coeff = numpy.abs(-1*tarspot_coeff)/numpy.abs(-1*tarspot_coeff).sum(),
    tfreq = numpy.where(-1 * tarspot_coeff < 0, 0.0, 1.0),
    tfreq_edge = numpy.where(-1 * tarspot_coeff < 0, 1.0, 0.0),
    tld = 1,
    tld_edge = 0,
    cycles = 0,
    d = numpy.repeat(1, len(tarspot_coeff)),
    lgroup_size = numpy.array([len(tarspot_coeff)]),
    sel_size = n_sel,
    algorithm = "hc_sa_set",
    algorithm_varg = None,
    #  {
