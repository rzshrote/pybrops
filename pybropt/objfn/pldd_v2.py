...
def pldd_v2(rsel, geno, wcoeff, tfreq, tld, gmap, lgroup, generations,
         mapfn = 'haldane', ldfn = 'r_sq', mem = None,
         mtype = 'tril', dtype = numpy.dtype('float64')):
    """
    Population Linkage Disequilibrium Distance (PLDD) objective function.
    ...
    for rst,rsp in row_ixfty(0, l, mem, None, None):    # row loop
        for cst,csp in col_ixfty(0, l, mem, rst, rsp):  # column loop
            ...
            ####################################################################
            score += wcoeff[rst:rsp].dot(diff_ld).dot(wcoeff[cst:csp])
