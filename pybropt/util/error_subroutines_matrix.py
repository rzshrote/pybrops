...
def cond_check_matrix_axis_len(mat, varname, axis, axislen, cond=(lambda mat: mat is not None)):
    if cond(mat):
        check_matrix_axis_len(mat, varname, axis, axislen)
##########
...
def check_matrix_dtype(mat, varname, dtype):
    if not numpy.issubdtype(mat.dtype, dtype):
        raise ValueError("'%s' must have a dtype of %s." % (varname, dtype))

def cond_check_matrix_dtype(mat, varname, dtype, cond=(lambda mat: mat is not None)):
    if cond(mat):
        check_matrix_dtype(mat, varname, dtype)
