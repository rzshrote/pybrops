...
class MOGM(GenomicMating):
    ...
    def objfn_platypus(self, sel, objcoeff = None, axissum = None, minimizing = True):
        """
        sel : list
            A list of selection indices corresponding to the configuration.
            Any input list will be converted to a numpy.ndarray and flattened,
            so make sure the flattening will result in the correct shape!
        """
        # convert 'sel' to numpy.ndarray and flatten; flatten is needed!
        sel = numpy.array(sel).flatten()

        # calculate MOGM scores
        mogm = self.objfn(
            sel,
            objcoeff = objcoeff,
            axissum = axissum,
        ...
        mogm_list = [e for e in mogm.flatten()]
