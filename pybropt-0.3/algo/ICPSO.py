...
class ICPSO(ParticleSwarmOptimization):
    ...
    def reset(self):
        ...
        # set to empty list
        self._sample = []
        self._velocity = [] # wtf: this should be initialized in super().reset()
    ...
    def optimize(self, objfn, stpfn, seed = None, nthreads = 0, minimize = True,
        ...
        **kwargs
        ...
        states = self._search_space.state
        ...
        while stpfn(iter):
            ...
            acoef_pbest = numpy.random.uniform(
                low = 0.0,
                high = self._pbest_comp,
                size = n
            )
            acoef_gbest = numpy.random.uniform(
                low = 0.0,
                high = self._gbest_comp,
                size = n
            )

            # calculate the new velocities
            vel = (self._inertia_wt * vel) + \
            ...
            improve_mask = score < pbest_score if minimize else score > pbest_score
            ...
            gbest_ix = pbest_score.argmin() if minimize else pbest_score.argmax()
